...
transitionMatrix :: forall model state cmd. (Eq state, Generic state)
                 => (GEnum FiniteEnum (Rep state), GBounded (Rep state))
                 => Markov model state cmd -> M
transitionMatrix markov = Matrix.fromList dim dim values
  where
    dim = length states + 1 -- + 1 for the stop/exit/sink state.
...
toMatrix :: forall state. Ord state
         => (Generic state, GEnum FiniteEnum (Rep state), GBounded (Rep state))
         => Map (state, Maybe state) Double -> M
toMatrix m = Matrix.fromList (pred dim) dim values
  where
    dim = length states + 1 -- We add one for the stop/sink state.
