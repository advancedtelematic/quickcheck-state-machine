...
module Test.Tasty.QuickCheck.Markov
  ( testProperty
  , successful
  ) where
...
data QCMarkov
  = forall model state cmd.
      ...
      ) => QCMarkov Tasty.TestName (Proxy state) (Markov model state cmd) QC.Property
...
testProperty name prop markov
  = Tasty.singleTest name $ QCMarkov name (Proxy @state) markov (QC.property (prop markov))
...
instance Tasty.IsTest QCMarkov where
  testOptions = pure
  ...
  run opts (QCMarkov name proxy markov prop) _yieldProgress = do
